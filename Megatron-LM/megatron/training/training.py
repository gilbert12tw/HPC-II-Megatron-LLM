...
def training_log(
    ...
    if iteration % args.log_interval == 0:
        ...
        # Calculate average loss before resetting (for custom output)
        avg_loss_for_display = None
        for key in total_loss_dict:
            if key not in [advanced_iters_key, skipped_iters_key, nan_iters_key]:
                if total_loss_dict[advanced_iters_key] > 0:
                    avg_loss_for_display = total_loss_dict[key].item() / float(
                        max(1, total_loss_dict[advanced_iters_key])
                    )
                    break
        
        # Add loss to log string
        for key in total_loss_dict:
            if key not in [advanced_iters_key, skipped_iters_key, nan_iters_key]:
        ...
        log_string += f' number of skipped iterations: {total_loss_dict[skipped_iters_key]:3d} |'
        log_string += ' number of nan iterations: {:3d} |'.format(total_loss_dict[nan_iters_key])
        
        # Add memory information to log (custom format)
        if torch.cuda.is_available():
            mem_gb = torch.cuda.memory_allocated() / (1024**3)
            max_mem_gb = torch.cuda.max_memory_allocated() / (1024**3)
            log_string += f' MemAllocated={mem_gb:.2f}GB MaxMemAllocated={max_mem_gb:.2f}GB |'
        
        # Calculate tokens/s for custom output format
        tokens_per_sec = 0.0
        if args.log_throughput and elapsed_time_per_iteration > 0:
            tokens_per_sec = (batch_size * args.seq_length) / elapsed_time_per_iteration
            log_string += f' Global Tokens/s: {tokens_per_sec:.2f} |'
        ...
        # Print custom formatted output on rank 0 only
        if torch.distributed.get_rank() == 0 and avg_loss_for_display is not None:
            if torch.cuda.is_available():
                mem_gb = torch.cuda.memory_allocated() / (1024**3)
                max_mem_gb = torch.cuda.max_memory_allocated() / (1024**3)
            else:
            ...
            print(f"[Step {iteration}/{args.train_iters}] Loss: {avg_loss_for_display:.4f} | "
                  f"Global Tokens/s: {tokens_per_sec:.2f} | "
...
def train(
    ...
    # Track warmup samples and actual training samples separately for precise throughput calculation
    warmup_samples = 0
    actual_training_samples = 0
    ...
    # Store the initial start iteration for warmup calculation (won't change like start_iteration does)
    initial_start_iteration = iteration
    ...
    while iteration < args.train_iters:
        ...
        if iteration in args.iterations_to_skip:
            ...
            # Count skipped iterations as warmup samples
        ...
        # Track warmup vs actual training samples for precise throughput calculation
        # Warmup is complete when CUDA graph warmup is done AND initial start iterations are complete
        if not warmup_complete:
            # Check if warmup is now complete
            # Warmup includes: cuda_graph_warmup_steps and initial start_iteration stabilization
            # Use initial_start_iteration (not start_iteration) as start_iteration changes during the loop
            warmup_threshold = max(args.cuda_graph_warmup_steps, initial_start_iteration + 1)
            if iteration > warmup_threshold:
                warmup_complete = True
                if training_start_time is None:
                    training_start_time = time.time()

        # Count samples in appropriate bucket
        if warmup_complete:
            actual_training_samples += iteration_sequences
        else:
            warmup_samples += iteration_sequences
    ...
    # Print final training summary on rank 0
    if mpu.get_data_parallel_rank() == 0:
        # Calculate total training time (wall clock from process start)
        total_time = time.time() - _TRAIN_START_TIME

        # Calculate actual training time (excluding warmup)
        if training_start_time is not None and warmup_complete:
            actual_training_time = time.time() - training_start_time
            initialization_time = total_time - actual_training_time
        else:
            # Fallback if warmup was never marked as complete
            actual_training_time = total_time
            initialization_time = 0.0

        # Calculate average tokens/s (two methods for comparison)
        total_tokens = args.consumed_train_samples * args.seq_length
        avg_tokens_per_sec_total = total_tokens / total_time if total_time > 0 else 0.0

        # Calculate throughput excluding warmup (more precise)
        if actual_training_samples > 0 and actual_training_time > 0:
            actual_training_tokens = actual_training_samples * args.seq_length
            avg_tokens_per_sec_training = actual_training_tokens / actual_training_time
        else:
            avg_tokens_per_sec_training = 0.0
        ...
        if torch.distributed.get_rank() == 0:
            ...
            print("="*70)
            print(f"Total time (wall clock): {total_time:.2f} seconds")
            print(f"Initialization time: {initialization_time:.2f} seconds")
            print(f"Actual training time (excluding warmup): {actual_training_time:.2f} seconds")
            print("-"*70)
            print(f"Total samples (including warmup): {args.consumed_train_samples:,}")
            print(f"Warmup samples: {warmup_samples:,}")
            print(f"Actual training samples: {actual_training_samples:,}")
            print("-"*70)
            print(f"Avg Global Tokens/s (total, including all overhead): {avg_tokens_per_sec_total:.2f}")
            print(f"Avg Global Tokens/s (excluding warmup): {avg_tokens_per_sec_training:.2f}")
            print(f"Peak GPU Mem (GB): {peak_mem_gb:.2f}")
            print("="*70 + "\n")
